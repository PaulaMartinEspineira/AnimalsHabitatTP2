...
import simulator.model.Carnivores;
...
public class Main {
	 ...
	 /* 
	  //La linea de comandos para ejecutar el modo batch es: -m batch -i resources/examples/ex1.json -o resources/tmp/myout.json -t 60.0 -dt 0.03 -sv
	  
	  
	  //Para añadir observador en modo bacth:
	  - creas clase que vaya a implementar al observador (en model!)
	  - si debe imprimir, acordarte de metodo extra que imprima
	  - añades en el main atributo observador
	  - añades comando en el parse y en el build_options
	  - inicializas el nuevo observador en modo batch
	  - lo imprimes
	  
	 
	  //Para añadir nueva tabla: 
	  - creas clase tablaXTableModel que extiende a abstractTableModel e implementa el observador 
	  - la añades en el mainWindow (o donde quieres que se vea)
	  
	  
	  //Para añadir nuevo animal/region: 
	  - fill_in_data del builder cambia la info del changeRegionsDialog 
	  - añadimos clase builder de la nueva region/animal 
	...
	*/
	...
	private static Boolean BoolCar=false;
	...
	//Atributo observador
	public static Carnivores carnivores;
	...
	private static Options build_options() {
		...
		// carnivores
				cmdLineOptions.addOption(Option.builder("car").longOpt("carnivores").build());
...
//METODOS PARSE
	...
	private static void parse_carnivores_option(CommandLine line) {
...
//METODOS DE EJECUCION
	private static void start_batch_mode() throws Exception {
		...
		if(BoolCar!=false) {
			carnivores = new Carnivores(controller);
		}
		controller.run(_time, _delta_time, _sv, out);
		if(BoolCar!=false) {
			carnivores.printInfo();
		}
		out.close();
	}
